...
public sealed class AgentRuntimeServicesTests
{
    ...
    public async Task CopilotToolFactory_ScheduleGroupTask_CreatesPersistedTask()
    {
        ...
        Assert.Equal(FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Active, stored.Status);
    ...
    public async Task CopilotToolFactory_ListScheduledTasks_MainGroupCanSeeAllGroups()
    {
        ...
        taskRepository.Stored.Add(new ScheduledTask(new TaskId("task-main"), mainGroup.Folder, mainChatJid, "Main task", ScheduleType.Once, "2026-03-11T00:00:00Z", TaskContextMode.Isolated, DateTimeOffset.Parse("2026-03-11T00:00:00Z"), null, null, FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Active, DateTimeOffset.UtcNow));
        taskRepository.Stored.Add(new ScheduledTask(new TaskId("task-team"), otherGroup.Folder, otherChatJid, "Team task", ScheduleType.Interval, "300000", TaskContextMode.Group, DateTimeOffset.UtcNow.AddMinutes(5), null, null, FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Paused, DateTimeOffset.UtcNow));
        taskRepository.Stored.Add(new ScheduledTask(new TaskId("task-done"), otherGroup.Folder, otherChatJid, "Done task", ScheduleType.Once, "2026-03-09T00:00:00Z", TaskContextMode.Isolated, null, DateTimeOffset.UtcNow.AddDays(-1), "Completed", FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Completed, DateTimeOffset.UtcNow.AddDays(-2)));
    ...
    public async Task CopilotToolFactory_TaskManagementTools_UpdatePersistedTaskStatus()
    {
        ...
        taskRepository.Stored.Add(new ScheduledTask(new TaskId("task-active"), group.Folder, chatJid, "Active task", ScheduleType.Interval, "300000", TaskContextMode.Group, DateTimeOffset.UtcNow.AddMinutes(5), null, null, FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Active, DateTimeOffset.UtcNow));
        taskRepository.Stored.Add(new ScheduledTask(new TaskId("task-paused"), group.Folder, chatJid, "Paused task", ScheduleType.Interval, "300000", TaskContextMode.Group, DateTimeOffset.UtcNow.AddMinutes(5), null, null, FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Paused, DateTimeOffset.UtcNow));
        ...
        Assert.Equal(FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Paused, taskRepository.Stored.Single(task => task.Id.Value == "task-active").Status);
        Assert.Equal(FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Cancelled, taskRepository.Stored.Single(task => task.Id.Value == "task-paused").Status);
    ...
    public async Task CopilotToolFactory_TaskManagementTools_RejectCrossGroupMutationForNonMainRequest()
    {
        ...
        taskRepository.Stored.Add(new ScheduledTask(new TaskId("task-other"), otherGroup.Folder, otherChatJid, "Other task", ScheduleType.Interval, "300000", TaskContextMode.Group, DateTimeOffset.UtcNow.AddMinutes(5), null, null, FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Active, DateTimeOffset.UtcNow));
        ...
        Assert.Equal(FireLakeLabs.NetClaw.Domain.Enums.TaskStatus.Active, taskRepository.Stored.Single().Status);
    }
